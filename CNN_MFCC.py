...
class SRDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        mfcc = librosa.feature.mfcc(y=soundData3, sr=sr, n_mfcc=20)
        mfcc = torch.from_numpy(mfcc)
        tempData = torch.zeros([1,8000])
        mfcc2 = mfcc.reshape(1, -1)
        if mfcc2.numel() < 8000:
            tempData[:,:mfcc2.numel()] = mfcc2[:,:]
        else:
            tempData[:] = mfcc2[:,:8000]
...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv1 = nn.Conv1d(1, 128, 10, 4)
